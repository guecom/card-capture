...
/**
 * `attention`·`researchProgress`는 서버가 목록에 실어 보내지만 `contracts/capture.ts`에는 없는
 * 필드다. 모듈이 그 둘을 계약 밖 값으로 다루는 것이 계약이므로, 여기서도 계약 밖 값으로 넣는다.
 */
type ServerExtras = { attention?: unknown; researchProgress?: unknown };
const brief = (over: Partial<BriefItem> & ServerExtras) => ({ captureId: 'c1', status: 'received', ...over } as BriefItem);
