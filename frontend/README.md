...
This directory owns the React + TypeScript + Vite production frontend. The root Pages entrypoint preserves query parameters and opens the built app under `docs/next/`; the retired standalone app no longer ships.
...
- Camera, OpenCV, Tesseract, upload/retry, brief, profile and post-processing actions retain the established payload and authority contracts behind typed services.
...
The Vite build writes only `docs/next/`. The public root `docs/index.html` is the query-preserving live entrypoint, and `config/public-runtime.json` is the single pinned public API source.
...
4. Camera/detector/OCR recovery inside the current app, including native-camera fallback.
