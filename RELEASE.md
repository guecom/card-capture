...
### v2.20.0 @ `f85f737` — 검증 2026-08-02 (agent:kairen.codex, Kairen-Ref: TSK-000496 / INT-000025) — **Code.gs·watcher 변경 / Web Push OFF**
...
- human gate: founder가 INT-000025의 ISS-000218 제외 전체 구현과 `PR, Merge, Release`, 이어 release·deployment까지 승인했다. 이 승인은 repository merge, tag, Pages, 기존 GAS deployment 갱신과 watcher 재기동을 포함한다. **sender secret의 특정 목적지 전달과 Android live Push 결과를 승인·검증한 것으로 해석하지 않는다.**
...
- GAS deployment: 기존 deployment와 공개 `/exec` URL을 유지한 채 release `Code.gs`로 **version 7**을 배포했다. 운영 safe probe는 `ping` → `ok:true`, legacy `notify` → `notification_channel_retired`, invalid Push sender → `sender_unauthorized`, 무효 토큰 `whoami`·`list`·`persondoc`·`search`와 POST `researchinstruction` → 각각 `invalid_token`, 미등록 GET action → `unknown_action`다. `DEEP_RESEARCH_ENABLED=true`, `PUSH_NOTIFICATIONS_ENABLED=false`로 배포했다. guest `owner_only`, target mismatch와 유효 owner-token read/write는 secret·운영 데이터 경계 때문에 `NOT RUN — HUMAN/EXTERNAL`이다. raw Script/deployment/registry ID와 token·private key는 이 기록에 남기지 않는다.
- Web Push: VAPID public key는 존재·형식 검증됐고 private registry는 owner-only·unshared이며 watcher sender 설정은 Windows DPAPI로 보호된다. 전용 sender dependency install·audit와 watcher safe health는 통과했다. 그러나 GAS `PUSH_SENDER_TOKEN`은 목적지별 secret 전달 승인을 기다려 **absent**이고 server Push는 OFF다. Android opt-in → 닫힌 앱 수신 → same-origin 열기 → revoke는 `NOT RUN — HUMAN/EXTERNAL`; machine PASS를 live PASS로 승격하지 않는다.
...
- 실기기·closure 판정: **미완료.** sender secret 목적지 승인, Push enable, Android opt-in·닫힌 앱 수신·tap-open·해제를 TSK-000155에서 확인할 때까지 TSK-000496은 In Progress이고 관련 live Issue도 In Progress다. 이 release evidence는 Task Done·MVP gate PASS·customer proof가 아니다. MVP build/testability gate comes before customer proof.
