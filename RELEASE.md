...
### v2.25.0 @ `1235078` — 검증 2026-08-04 (agent:kairen.claude, Kairen-Ref: TSK-000220 / INT-000030) — **INT-000030 다섯 표면 · Code.gs 변경 / GAS 재배포 미실행 · GitHub Release 미발행 · actual phone NOT RUN**
...
- repository: annotated release tag `v2.25.0`(tag object `917cb40718441babda11cd35e55ca53fb0fbf7d5`) → squash merge commit `123507819cd0245b21a99b5d4ee976f8cd21983b`(main). 구현 PR은 [#83](https://github.com/guecom/card-capture/pull/83)이고 승인·검증된 exact PR head는 `f63c11bae08f170a34d4dd9758f54dc0da799638`다. base는 `05008009d78a37c8acbefc37ab0f324a0bb39869`(PR #82의 v2.24.1 evidence merge)이며 `v2.24.1` tag 자체는 `8b981875d1c190104e956bd8e88b626a44edda71`을 가리킨다 — **base commit과 직전 release tag는 같은 commit이 아니다.** 기존 tag는 움직이지 않았다.
  - **GitHub Release는 아직 발행되지 않았다.** tag는 origin에 있지만(`refs/tags/v2.25.0` → `917cb407…` → `1235078…`) `gh release view v2.25.0`은 `release not found`이고 목록의 Latest는 여전히 `v2.24.1`이다. v2.19.0부터 v2.24.1까지는 전부 정식 Latest release로 발행됐다 — 이 release만 그 단계가 비어 있다. 아래 "남은 사람 단계" 1번이다.
...
- **GAS deployment: 필요하지만 미실행.** `0500800…..1235078…`에서 `Code.gs`가 **+14/−4줄** 바뀌었다 — research mode allowlist에 `quick`을 넣고(`RESEARCH_MODES_ = ['quick', 'standard', 'deep_evidence_graph']`) 그 예산을 `branchCap 4 / timeCapMinutes 10`으로 정했다(deep은 24/90, standard는 10/30 그대로). **Apps Script 재배포는 사람 단계이고 실행되지 않았다. 그래서 live 서버는 지금 이 release 이전 코드를 돌리고 있다.**
  - 그 구간의 실제 동작: `빠른 조사` 요청은 옛 서버 allowlist에 `quick`이 없어 **요청 전체가 `bad_research_request`로 거절된다**(촬영 업로드 경로에서는 사진까지 함께 거절된다). 클라이언트가 같은 요청의 `mode` 한 칸만 `standard`로 바꿔 **딱 한 번** 다시 보낸다. 그래서 **사용자는 조사를 정상적으로 받고**, 내려갔다는 사실은 개발자 채널(`research-telemetry.ts` — 기억 속에만 사는 read model, 저장소·주소창·화면 어디에도 쓰지 않음)에 `degraded`로만 남는다.
  ...
  - `config/public-runtime.json`, Script Properties, credential, production data는 바꾸지 않았다.
  - **agent는 live 배포 상태를 확인할 수 없다.** `ping`은 build 표식을 돌려주지 않고(`{ok, service, time}`뿐), 이번에 바뀐 코드가 닿는 경로는 전부 token 뒤에 있으며 유효 owner token은 agent가 쓰지 않는다. 실행한 무해 probe는 둘이다 — `?action=ping` → `{"ok":true,"service":"card-capture",…}`, 미등록 GET action → `{"ok":false,"error":"unknown_action"}`. 이것은 **서버가 살아 있다**는 뜻이지 **이 release의 `Code.gs`가 배포됐다**는 뜻이 아니다.
...
- **남은 사람 단계 (이 순서대로)**:
  1. **GitHub Release `v2.25.0` 발행.** tag는 origin에 있는데 release가 없다. draft·prerelease가 아닌 Latest 정식 release로 낸다. 나머지 단계와 독립적이므로 먼저 닫는다.
  2. **Apps Script 재배포** — vault `CardCapture_Setup.md`의 클릭 단위 절차. 이 단계 전까지 `빠른 조사`는 서버에서 거절되고 조용히 `일반 조사`로 나간다(위 GAS deployment 항목).
  3. **Script Property `DEEP_RESEARCH_ENABLED` 확인·결정.** `true`가 아니면 `깊은 조사`는 계속 `지금은 못 골라요`다. 켤지 말지는 사람이 정한다. 2번과 순서를 바꾸지 않는다 — 옛 코드 위에서 이 값만 켜면 `깊은 조사`는 열리는데 `빠른 조사`는 여전히 내려간다.
  4. **재배포 뒤 behavior 재검증**(`Release 절차` 6단계). 폰에서 유효 owner token으로 `빠른 조사` 한 번이 `bad_research_request` 없이 접수되는지, `깊은 조사`가 범위 1개 이상에서 접수되는지, 배포된 앱의 설정 화면이 `버전 2.25.0 · 빌드 src-d027b28abbb1`을 말하는지 확인한다.
...
- rollback: **지금은 PR revert 하나로 충분하다.** `Code.gs`가 바뀌었지만 **배포되지 않았으므로** live 서버는 이 release 이전 코드 그대로이고 되돌릴 서버 상태가 없다. PR #83 merge commit `1235078…`을 revert하고 같은 Pages 절차로 재배포하면 끝이다. 기존 tag를 움직이거나 재사용하지 않는다. Script Property·credential·production data·watcher rollback은 없다.
  - **위 2번(GAS 재배포)을 실행한 뒤에는 revert 하나로 부족하다.** 그때는 Apps Script 배포 관리에서 이전 version으로 함께 되돌리고 behavior probe를 다시 돌려야 한다(`Rollback` 절). **배포된 새 서버 + 되돌린 옛 앱** 조합은 검증된 적이 없다 — 옛 앱은 `quick`을 보내지 않으므로 기능상 조용하겠지만, 그것은 검증이 아니라 추측이다.
  - 되돌리면 사라지는 것: 다섯 표면 전부와 `subgrid` 정렬. 그리고 `v2.24.1` 기준선으로 돌아가므로 **PC 첫 방문에서 `AI 조사 요청` 작성 자리가 다시 DOM에서 통째로 사라지고 화면 맨 위에 `링크 설정이 필요해요` 전면 경고가 돌아온다**(base `0500800…`의 `App.tsx`에서 실제로 그 상태임을 확인했다).
