...
## [Unreleased] — 사람 프로필 링크·액션·입력 UX 정리 (branch `agent/tsk-000220-person-ux-polish`, Kairen-Ref: ISS-000058, ISS-000066, ISS-000084, TSK-000220)
...
- **설정 문구를 사용자 관점으로 바꾼다**: `연결과 경계` 같은 내부 용어 대신 `내 앱 설정`, `이 기기`, `사용자·연결 정보 편집`, `데이터가 저장되는 곳`처럼 실제로 무엇을 확인하고 바꾸는지 설명한다. API 주소와 토큰은 `고급 설정` 안에 유지한다.
- **상단 헤더와 새로고침을 정돈한다**: 긴 `Kairen Card Capture 모바일 메모리` 표기를 `Card Capture`로 줄이고, 떠 있는 아이콘 버튼 대신 각 기록 화면 안의 `최신 상태 확인` 보조 동작으로 배치한다. `Live contract`, `Owner recall` 같은 내부 영문 라벨도 `최근 명함`, `빠른 검색`으로 바꾼다.
