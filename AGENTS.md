...
3. **secret 금지.** 토큰 값, `TOKENS` JSON, Drive folder ID, 실캡처 데이터(이미지·capture.json·brief), Person 개인정보를 이 저장소에 넣지 않는다. `config/public-runtime.json`의 `apiUrl`(GAS exec URL)만 예외로 허용된 공개값이다. 이 JSON field가 `frontend/vite.config.ts`가 읽어 React 앱의 pinned endpoint로 주입하는 **유일한 원본**이며, 누락되거나 잘못되면 빌드가 `public_runtime_api_missing`으로 실패한다.
